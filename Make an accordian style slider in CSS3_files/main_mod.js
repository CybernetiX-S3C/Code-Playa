...
code_mirror_html = CodeMirror.fromTextArea( $('#html textarea')[0], {
	...
	onChange: function(instance, changes) {
		...
		if($save_timeline !== true && ($new_content != $ohc && should_render() || $slide == $slides-1))
		{
			update_live_preview();
		}
...
code_mirror_css = CodeMirror.fromTextArea( $('#css textarea')[0], {
	...
	onChange: function(instance, changes) {
		...
		if($save_timeline !== true && ($new_content != $occ && should_render() || $slide == $slides-1))
		{
			update_live_preview();
		}
...
code_mirror_js = CodeMirror.fromTextArea( $('#js textarea')[0], {
	...
	onChange: function(instance, changes) {
		...
		if($save_timeline !== true && ($new_content != $ojc && should_render() || $slide == $slides-1))
		{
			update_live_preview();
		}
...
update_live_preview = function() {
	...
	if($("#output iframe").length > 1)
	{
		var multiple_iframes = true;
		if(typeof active_iframe == "undefined")
		{
			active_iframe = "two";
		}
		var iframe_one = $("#output iframe.one");
		var iframe_two = $("#output iframe.two");
	}
	else
	{
		var iframe_one = $("#output iframe");
		var multiple_iframes = false;
	}
	...
	if(!multiple_iframes)
	{
		iframe_one.replaceWith('<iframe id="docCanvas" class="one" src="' + BASE_URL + '/iframe.html" width="1000" height="800"></iframe>');
	}
	else if(active_iframe == "two")
	{
		iframe_one.replaceWith('<iframe id="docCanvas" class="one" style="z-index: -1;" src="' + BASE_URL + '/iframe.html" width="1000" height="800"></iframe>');
	}
	else if(active_iframe == "one")
	{
		iframe_two.replaceWith('<iframe id="docCanvas2" class="two" style="z-index: -1;" src="' + BASE_URL + '/iframe.html" width="1000" height="800"></iframe>');
	}
	...
	if ( js_code ) prepared_source = prepared_source.replace('</body>', '<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js" type="text/javascript"></script>' + js + '</body>');
	// console.log(prepared_source);

	if(!multiple_iframes || active_iframe == "two")
	{
		ocontent[0].open();
		ocontent[0].write(prepared_source);
		ocontent[0].close();
		
		if(multiple_iframes) setTimeout(swap_iframes, 300);
	}
	else if(active_iframe == "one")
	{
		ocontent_two[0].open();
		ocontent_two[0].write(prepared_source);
		ocontent_two[0].close();
...
function swap_iframes()
{
	...
	{
		//alert("swapping to one");
		$("#output iframe.one").css("zIndex", "1");;
		$("#output iframe.two").css("zIndex", "-1");
		active_iframe = "one";
		ocontent_two[0].open();
		ocontent_two[0].write(old_prepared_source);
		ocontent_two[0].close();
	}
	else if(active_iframe == "one")
	{
		//alert("swapping to two");
		$("#output iframe.two").css("zIndex", "1");;
		$("#output iframe.one").css("zIndex", "-1");;
		active_iframe = "two";
		ocontent[0].open();
		ocontent[0].write(old_prepared_source);
		ocontent[0].close();
	}
}
